...
class PageStructureTests(TestCase):
    def test_index_has_single_h1_skip_link_and_main(self):
        ...
        self.assertIn("Birlikte karar verelim.", html)
    ...
    def test_slogan_is_consistent_everywhere(self):
        for old in ("kalabalık karar versin", "Sen sor, birlikte karar verelim"):
            offenders = [str(path.relative_to(TEMPLATE_DIR)) for path in TEMPLATE_DIR.rglob("*.html") if old in path.read_text(encoding="utf-8")]
            self.assertEqual(offenders, [], old)
        html = self.client.get("/").content.decode()
        self.assertIn('<meta name="description" content="Birlikte karar verelim.">', html)
        self.assertIn('<meta property="og:description" content="Birlikte karar verelim.">', html)
        self.assertRegex(html, r'<h1 class="page-title">Birlikte karar verelim\.</h1>')
